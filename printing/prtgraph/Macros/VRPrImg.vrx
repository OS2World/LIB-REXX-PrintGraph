...
Init:
    ...
    if method = "PrintObjectImage" then
        call VRSet "DT_12", "Caption", "Object"

    if method = "PrintWinImage" then
        call VRSet "DT_12", "Caption", "HWND"
...
PB_accept_Click: 
    ...
    tag_str = filename',' left',' bottom',' right',' top',' line',' lc',' title  ");"
...
*/
PB_File_Click: 
    FileName = VRFileDialog( VRWindow(), "Choose Bitmap", "Open", "*.bmp", , ,  )
    if FileName <> '' then
        call VRSet "EF_File", "Value", FileName
    drop FileName
return
